...
async def sedlyfsir(event):
    ...
    if Config.CUSTOM_START is None:
        text_me = (f"**Hai. {firstname} , Saya {bot_username}.** \n"
               f"`Saya seorang bot obrolan managed by` [Master](tg://user?id={oknoob}) \n"
               f"**Kirim sesuatu yang anda sampaikan ke tuan saya.** \n"
               f"**Terima kasih❤**")
    else:
        text_me = f"{oksir}"
    formaster = "Halo tuan, ada yang bisa saya bantu?"
    if event.sender_id == Config.OWNER_ID:
        ok = await chatbot.send_message(event.chat_id, message=formaster, buttons = [
             [custom.Button.inline("Perintah Untuk Owner.", data="cmds")],
...
async def help(event):
    if event.query.user_id is not Config.OWNER_ID:
        await event.edit(
            "Hai bosku kamu bisa join channel dan group. \nTerima kasih sudah menghubungi saya.",
            buttons=[
                [Button.url("Channel Memes 📺", "t.me/MemesRpi")],
                [Button.url("Group Fun ❓", "t.me/RPIrandompeopleindonesia")],
            ],
        )
